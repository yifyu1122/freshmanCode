...
class linkedList{
	...
	public:
		...
		void output(ofstream &file){
			temp = head;
			file << "C S  S T U D E N T" << endl;
			if(isEmpty()){
				if(temp->readPtr() == NULL){ // When there's only one node in list
				    file <<  "|| Rank ||         Name        || Number || Eng || Cmp || Total ||"  << endl
					<< setw(7) << "1";
...
class linkedList2{
	...
	public:
		...
		void output(ofstream &file){
			temp = head;
			file << "P E  S T U D E N T" << endl;
			if(isEmpty()){
				if(temp->readPtr() == NULL){ // When there's only one node in list
				    file <<  "|| Rank ||         Name        || Number || Run ||"  << endl
					<< setw(7) << "1";
			        file << setw(23) << temp->readName()
						 << setw(10) << temp->readNo() 
						 << setw(7) << temp->readRun()  << endl;
			    ...
			    for(int i=0; i<count; i++) {
					...
					file << setw(23) << nodes[i].readName()
						 << setw(7) << nodes[i].readNo()
						 << setw(10) << nodes[i].readRun() << endl;
			    }
			}
			else {  // When head is empty
				file << "Database is NULL!" << endl;
